...
- **TWO SEPARATE FAILURE CLASSES, and the distinction decides priority.** (1) REFUSALS — counterbore, shell beyond a box, fillet beyond a box. These were masked by the OCCT fallback and are visible now; honest, and they need real kernel capability (W2/W3/W5/W8). (2) A SILENT WRONG SOLID — successive bores. Never refused, so the fallback never fired and this was ALWAYS wrong, before and after. Root cause proven (msgbox #329): `ops.rs:499 region_inside()` intersects a half-plane or disk from every face of `other`, which only holds while `other` is convex; a prior bore's cylinder wall then contributes `intersect_disk`, collapsing the region so the next bore's floor probes outside and `keep_disk` drops it. Floors lost == prior-bore count, confirmed 6/6. Class (2) outranks class (1): with no fallback, "never return a wrong solid" is all that protects a student
- The gates cannot see class (2): `hole-corners` is a THROUGH bore with no floor to lose, and there is no blind multi-bore fixture anywhere. 61/61 green is not evidence here
