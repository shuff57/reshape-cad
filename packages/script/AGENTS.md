...
## OVERVIEW
reSHape Script interpreter, ModelDoc types, and the emitters that turn docs back into code (script round-trip, FreeCAD command transpile).
...
├── transpile.mjs          # Script -> fc-commands for the FreeCAD wasm engine
...
├── hull.ts                # Own convex hull; OCCT ships none in any build
...
| Script -> FreeCAD commands | transpile.mjs STATEMENTS dispatch table |
| Face naming across booleans | topo-name.ts (algebra) + topo-history.ts (OCCT queries) |
| Docs copy | reshape-docs.ts; coverage rule: every DSL call in at least two examples |

## CONVENTIONS
- The interpreter only builds a ModelDoc. No kernel calls, no geometry, no drawing; occt-build.ts consumes the doc.
...
- transpile.mjs is string-in/string-out, zero dependencies. Aliases (box vs cuboid) fold into STATEMENTS, one row per name.
- Subpath exports only, per package.json; index.ts re-exports the full surface.

## ANTI-PATTERNS
- VOCABULARY and the fns table are keyed 1:1 by tsc; docs, transpiler, and studio all read the same list. Adding a word means touching all of them or the slice is rejected.
